...
| **Budget export API routes — pre-Auth gating gap** | `/api/budget/[budgetId]/csv`, `/topsheet-pdf`, `/detail-pdf` are reachable by URL by anyone who knows a budgetId. The drawer buttons are gated client-side (producer-only via the budget page guard), but server-side has no JWT/session to validate pre-Auth. `readStoredViewerRole()` is browser-only. Acceptable for v1 dogfood (Tyler + Kelly only). Sealed by Auth day's full server-side guard pattern alongside the #24 RLS pass. Each route file carries an inline TODO marker. |
