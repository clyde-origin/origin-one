...
| **`cloneBudget` duplication — Prisma + Supabase** | Two implementations of the same deep-copy logic: `packages/db/src/clone-budget.ts` (Prisma, used by seed) and `apps/back-to-one/src/lib/db/queries.ts:createBudgetByClone` (Supabase, used by the picker). Introduced PR 11 (#68, Apr 27) — back-to-one's `.env.local` lacks `DATABASE_URL` so the Prisma helper isn't reachable from the app, and adding a Next.js API route just for this op was over-scoped for a one-off. Topological pass and "skip expenses per spec §7.2" are duplicated. Revisit when API routes proliferate (likely Auth or after) — at that point, fold both into a single shared call site behind an API route. Not v1-blocking. |
| **Budget reference HTML missing Frames D + E + F** | `apps/back-to-one/reference/budget-page.html` is the canonical reference per CLAUDE.md ("the reference file is the spec"). PRs 11 + 12 shipped three new surfaces — TemplatePicker, BudgetSettingsSheet, VariableInspectTooltip — without back-filling the reference. Real gap; restores reference-first discipline. Title suggestion: `docs(reference): add Frames D + E + F for picker + settings + variable tooltip`. Sequence: queue as a small follow-up PR after PR 14 ships. Not v1-blocking. |
| **VariableInspectTooltip positioning — fixed-center, not row-anchored** | PR 12 (#69, Apr 27) renders the tooltip at `bottom: 40%` of the viewport rather than anchored to the tapped row. Anchored positioning needs DOM measurement (`getBoundingClientRect` + scroll-aware repositioning). Marginal UX win for meaningful complexity. Tag for post-dogfood polish if a producer asks. |
| **Topsheet renders as two parallel components** | `TopsheetContent.tsx` (HTML primitives, on-screen drawer + side panel) and `TopsheetPdfDocument.tsx` (`@react-pdf/renderer` primitives, server-side export). Spec §6.7 framed this as "single component, two targets," but `@react-pdf` has its own primitive layer (`<Document>/<Page>/<View>/<Text>`) — you can't pass HTML through it. Both consume the same data shape and the same `compute.ts` pipeline (so totals never diverge), but visual styling is maintained manually. Alternative path (puppeteer / headless Chrome rendering `TopsheetContent`'s DOM) was rejected as 5–10× heavier on Vercel Function memory + cold start for marginal DRY win. Confirmed pattern from PR 13 (#TBD, Apr 27). |
| **Budget export API routes — pre-Auth gating gap** | `/api/budget/[budgetId]/csv`, `/topsheet-pdf`, `/detail-pdf` use `SUPABASE_SERVICE_ROLE_KEY` server-side, bypassing RLS — anon has no grants on public schema, so anon-key reads fail with 42501. Producer-only access is enforced at the UI layer (the drawer buttons live on the producer-gated budget page) until Auth day's session-aware middleware lands. Anyone who knows a `budgetId` can hit these routes by URL pre-Auth — acceptable for v1 dogfood (Tyler + Kelly only). Sealed alongside the #24 RLS pass when Auth lands. The service-role key MUST stay server-only — never use `NEXT_PUBLIC_` prefix for it. |
