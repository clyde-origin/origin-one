...
| Rate-unit semantics + math | **Schema landed 2026-04-26 in `feat/rate-unit-schema` (budget arc PR 3): `RateUnit` enum (`'day' \| 'hour'`) and `CrewTimecard.rateUnit` column, backfilled all 124 non-null seed rows to `'day'` (lowest seed rate is $250 — every existing row is a day rate). EntryCard now offers a HOUR/DAY chip selector with dynamic "Rate (per hour/day)" label; new entries default to `'hour'`. Math in `timecardTotal` / ProducerOverview is intentionally NOT yet fixed — that lands in PR 6 of the budget arc (`feat/budget-library`) where vitest is available so the fix ships verified via `computeExpenseUnits` helpers.** Until PR 6, the math remains the same broken state described in the original issue. Do not "fix" the math in isolation. |
