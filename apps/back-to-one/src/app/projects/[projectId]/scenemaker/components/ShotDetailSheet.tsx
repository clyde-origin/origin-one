  ...
  return (
    <>
      ...
      <div style={{ padding: '0 20px 14px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="font-mono" style={{ fontSize: '0.5rem', color: accent, letterSpacing: '0.08em', marginBottom: 5 }}>{shot.shotNumber}</div>
        <div style={{ fontSize: '1rem', fontWeight: 800, letterSpacing: '-0.02em', color: '#dddde8' }}>{shot.description || '—'}</div>
      </div>
      <div style={{ padding: '14px 20px', display: 'flex', flexDirection: 'column', gap: 11 }}>
        <div className="flex items-start gap-3">
          <span className="font-mono uppercase flex-shrink-0" style={{ fontSize: '0.44rem', color: '#62627a', letterSpacing: '0.08em', width: 68, paddingTop: 2 }}>Size</span>
          <span className="capitalize" style={{ fontSize: '0.76rem', fontWeight: 600, color: '#dddde8' }}>{shot.size?.replace(/_/g, ' ') || '—'}</span>
        </div>
        <div className="flex items-start gap-3">
